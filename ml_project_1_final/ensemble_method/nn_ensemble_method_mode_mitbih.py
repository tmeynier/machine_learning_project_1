...
Y_test = np.array(df_test[187].values).astype(np.int8)[np.newaxis]

# make predictions
yhats = np.asarray([model.predict(X_test) for model in models])
# argmax across classes
argmax = np.argmax(yhats, axis=2)
...
print(accuracy_str)
# precision tp / (tp + fp)
precision = precision_score(Y_test[0,:], outcomes[0,:], labels=range(5), average='weighted')
precision_str = 'Precision: %f' % precision 
print(precision_str)
# recall: tp / (tp + fn)
recall = recall_score(Y_test[0,:], outcomes[0,:], labels=range(5), average='weighted')
recall_str = 'Recall: %f' % recall
print(recall_str)
# f1: 2 tp / (2 tp + fp + fn)
f1 = f1_score(Y_test[0,:], outcomes[0,:], labels=range(5), average='weighted')
f1_str = 'F1 score: %f' % f1 
print(f1_str)

# Write output results in file
out = open("output_nn_ensemble_method_mode_mitbih.txt","w") 
